...
#define DEV_ADDR_BASE(intLineNo, devNo)	(DEV_REGS_START + (((intLineNo) - 3) * 0x80) + ((devNo) * 0x10))

#define DEVICE_N_FROM_REGSTATUS(regStatusAddr)  \
        ((((regStatusAddr) - DEV_REGS_START) % 0x80)/ 0x10)

#define IS_DEVICE(address) \
        (((memaddr)(address)) >= (DEV_REGS_START) && \
        (((memaddr)(address)) < (TERMINAL0ADDR + (DEV_REG_SIZE * DEV_PER_INT))))

#define IS_DISK_DEVICE(address) \
        ((((memaddr)(address)) >= (DEV_REGS_START) && \
        (((memaddr)(address)) < (DEV_REGS_START + (DEV_REG_SIZE * DEV_PER_INT)))))

#define IS_TAPE_DEVICE(address) \
        ((((memaddr)(address)) >= (DEV_REGS_START + (DEV_REG_SIZE * DEV_PER_INT))) && \
        (((memaddr)(address)) < (DEV_REGS_START + (DEV_REG_SIZE * DEV_PER_INT * 2))))

#define IS_ETHERNET_DEVICE(address) \
        ((((memaddr)(address)) >= (DEV_REGS_START + (DEV_REG_SIZE * DEV_PER_INT * 2))) && \
        (((memaddr)(address)) < (DEV_REGS_START + (DEV_REG_SIZE * DEV_PER_INT * 3))))

#define IS_PRINTER_DEVICE(address) \
        ((((memaddr)(address)) >= (DEV_REGS_START + (DEV_REG_SIZE * DEV_PER_INT * 3))) && \
        (((memaddr)(address)) < (DEV_REGS_START + (DEV_REG_SIZE * DEV_PER_INT * 4))))
...
//#define GET_TODLOW ((*((U32 *)BUS_TODLOW) / (*(U32 *)BUS_TIMESCALE))-(TODLOW_to_decrease+50))
...
#define IS_KU_MODE(status)	(!((status) & STATUS_KUp))
...
#define INTERRUPT_TYPE(int_no)          (1 << ((int_no) + 8))
